export const homeInfluencers = [
  { src: "https://i.ibb.co/nq55PhZD/618511856-1441106430977518-1909843670823778383-n.jpg", height: "h-32" },
  { src: "https://i.ibb.co/fdcHktw2/595314692-2789237971426224-2629402992759056661-n.jpg", height: "h-28" },
  { src: "https://i.ibb.co/jvpj442N/271974866-5195343743897489-1185375077007356153-n.jpg", height: "h-48" },
  { src: "https://i.ibb.co/G4N2vfjk/451656152-3817656788505804-8821984264431332180-n.jpg", height: "h-44" },
  { src: "https://i.ibb.co/5gxMrJBW/626302384-1807681613230382-8649067781445416988-n.jpg", height: "h-48" },
  { src: "https://i.ibb.co/d4VrJFLz/473079489-1799843420753034-726780744381604068-n.jpg", height: "h-36" },
  { src: "https://i.ibb.co/jZHNTXHB/548183374-803517145699947-1055836045542047515-n-1.jpg", height: "h-40" },
  { src: "https://i.ibb.co/7hfj6mP/2sds4.png", height: "h-32" },
  { src: "https://i.ibb.co/m5Kh92n7/342784547-2690681411075051-4136922621935771916-n.jpg", height: "h-44" },
  { src: "https://i.ibb.co/B2Twp4Jd/488898903-3004828386363702-1003182827513052565-n.jpg", height: "h-44" },
  { src: "https://i.ibb.co/6J1wvp0m/508108834-4296502983955751-8425227864715454658-n.jpg", height: "h-36" },
  { src: "https://i.ibb.co/KxL0V4D0/482242748-23986036960986271-1620725418600049193-n.jpg", height: "h-40" },
];

export const influencers = [
  { src: "https://i.ibb.co/B2Twp4Jd/488898903-3004828386363702-1003182827513052565-n.jpg", height: "h-44" },
  { src: "https://i.ibb.co/7hfj6mP/2sds4.png", height: "h-32" },
  { src: "https://i.ibb.co/KxL0V4D0/482242748-23986036960986271-1620725418600049193-n.jpg", height: "h-40" },
  { src: "https://i.ibb.co/RpxCrW4V/487225120-1198729698273892-1454770218628643691-n.jpg", height: "h-44" },
  { src: "https://i.ibb.co/5gxMrJBW/626302384-1807681613230382-8649067781445416988-n.jpg", height: "h-48" },
  { src: "https://i.ibb.co/6J1wvp0m/508108834-4296502983955751-8425227864715454658-n.jpg", height: "h-36" },
  { src: "https://i.ibb.co/m5Kh92n7/342784547-2690681411075051-4136922621935771916-n.jpg", height: "h-44" },
  { src: "https://i.ibb.co/gZyhYJBN/OIP.webp", height: "h-40" },
  { src: "https://i.ibb.co/jZHNTXHB/548183374-803517145699947-1055836045542047515-n-1.jpg", height: "h-40" },
  { src: "https://i.ibb.co/fdcHktw2/595314692-2789237971426224-2629402992759056661-n.jpg", height: "h-28" },
  { src: "https://i.ibb.co/G4N2vfjk/451656152-3817656788505804-8821984264431332180-n.jpg", height: "h-44" },
  { src: "https://i.ibb.co/nq55PhZD/618511856-1441106430977518-1909843670823778383-n.jpg", height: "h-32" },
  { src: "https://i.ibb.co/jvpj442N/271974866-5195343743897489-1185375077007356153-n.jpg", height: "h-48" },
  { src: "https://i.ibb.co/d4VrJFLz/473079489-1799843420753034-726780744381604068-n.jpg", height: "h-36" },
  { src: "https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?w=400&h=550&fit=crop", height: "h-44" },
  { src: "https://images.unsplash.com/photo-1580489944761-15a19d654956?w=400&h=420&fit=crop", height: "h-32" },
  { src: "https://images.unsplash.com/photo-1539571696357-5a69c17a67c6?w=400&h=360&fit=crop", height: "h-36" },
  { src: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=400&h=520&fit=crop", height: "h-40" },
  { src: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=400&h=460&fit=crop", height: "h-32" },
  { src: "https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=400&h=580&fit=crop", height: "h-48" },
  { src: "https://images.unsplash.com/photo-1552058544-f2b08422138a?w=400&h=440&fit=crop", height: "h-36" },
  { src: "https://images.unsplash.com/photo-1567532939604-b6b5b0db2604?w=400&h=500&fit=crop", height: "h-40" },
];

export const testimonials = [
  {
    name: "محمد عجينة",
    text: "شريك النجاح ومؤسس شركة ريبيكيا، تعلمت منه الكثير حول القيادة الاستراتيجية وبناء الشركات الناشئة من الصفر.",
    img: "https://i.ibb.co/G4N2vfjk/451656152-3817656788505804-8821984264431332180-n.jpg",
    role: "موسس شركة ريبكيا"
  },
  {
    name: "طه فايد",
    text: "مستشار متميز في تجربة العميل، تعلمت منه الكثير حول تصميم الخدمات التي تضع المستخدم في المقام الأول.",
    img: "https://i.ibb.co/nq55PhZD/618511856-1441106430977518-1909843670823778383-n.jpg",
    role: "مؤسس شركة غدوة"
  },
  {
    name: "دكتورة ميسم مجدى",
    text: "أحمد ليس مجرد مدير منتجات، بل هو شريك في النجاح. خلال عملنا معاً، أثبت مهارة استثنائية في حل المشكلات المعقدة من خلال التخطيط الاستراتيجي والتصميم. التزامه بالجودة والمواعيد يجعله الخيار الأول دائماً.",
    img: "https://i.ibb.co/fdcHktw2/595314692-2789237971426224-2629402992759056661-n.jpg",
    role: "مديرة شركة ابن سينا"
  },
  {
    name: "أحمد رمضان",
    text: "من أفضل خبراء أنظمة التصميم، رؤيته الفنية ساهمت في تطوير مهاراتي في بناء واجهات متسقة وقابلة للتوسع.",
    img: "https://i.ibb.co/jvpj442N/271974866-5195343743897489-1185375077007356153-n.jpg",
    role: "مدير تصميم واجهات"
  },
  {
    name: "سوسن احمد",
    text: "تعلمت منها الكثير حول صناعة المحتوى وكيفية بناء حضور رقمي مؤثر وجذاب.",
    img: "https://i.ibb.co/5gxMrJBW/626302384-1807681613230382-8649067781445416988-n.jpg",
    role: "كونتت كريتور"
  },
  {
    name: "عادل الخميسى",
    text: "من أفضل من تعاملت معهم في تطوير البرمجيات، لديه رؤية تقنية عميقة واهتمام كبير بجودة الكود.",
    img: "https://i.ibb.co/6J1wvp0m/508108834-4296502983955751-8425227864715454658-n.jpg",
    role: "سينور سوفت وير"
  },
  {
    name: "احمد جعفر",
    text: "شخصية ملهمة ساعدتني في فهم أعمق للتقنيات الزراعية الحديثة وكيفية توظيفها في حل المشكلات البيئية.",
    img: "https://i.ibb.co/KxL0V4D0/482242748-23986036960986271-1620725418600049193-n.jpg",
    role: "موسس شركة فسيلة"
  },
  {
    name: "عبدالرحمن داود",
    text: "تعلمت منه كيف يمكن للإبداع البصري أن يغير مسار أي منتج رقمي ويجعله أكثر جاذبية للمستخدمين.",
    img: "https://i.ibb.co/RpxCrW4V/487225120-1198729698273892-1454770218628643691-n.jpg",
    role: "موسس شركة لقططة"
  },
  {
    name: "محمد السقا",
    text: "رؤيته في التصميم الجرافيكي والهوية البصرية كانت ملهمة جداً بالنسبة لي في بناء واجهات مستخدم متسقة.",
    img: "https://i.ibb.co/B2Twp4Jd/488898903-3004828386363702-1003182827513052565-n.jpg",
    role: "سينور جرافيكس دزاين"
  },
  {
    name: "محمود صفوت",
    text: "ساهم في تبسيط مفاهيم تطوير الأعمال المعقدة وجعلها أكثر قابلية للتطبيق العملي في المشاريع التعليمية.",
    img: "https://i.ibb.co/7hfj6mP/2sds4.png",
    role: "سينور بزنيسس ديفلوبر ف ischool"
  },
  {
    name: "محمد جاوويس",
    text: "شخصية ملهمة في مجال ريادة الأعمال التعليمية، ساهم بشكل كبير في تطوير رؤيتي لبناء منصات تعليمية ناجحة.",
    img: "https://i.ibb.co/m5Kh92n7/342784547-2690681411075051-4136922621935771916-n.jpg",
    role: "موسس شركة ischool"
  },
  {
    name: "دكتور احمد العربى",
    text: "تعلمت منه الكثير في بداياتي، وكان له أثر كبير في تشكيل رؤيتي للتصميم الرقمي والعمل الأكاديمي.",
    img: "https://i.ibb.co/gZyhYJBN/OIP.webp",
    role: "خبير تصميم رقمي"
  },
  {
    name: "احمد النجار",
    text: "خبير في تجربة المستخدم، عملنا معاً على عدة مشاريع وكان دائماً يضيف قيمة استثنائية من خلال رؤيته المبتكرة.",
    img: "https://i.ibb.co/d4VrJFLz/473079489-1799843420753034-726780744381604068-n.jpg",
    role: "موسس شركة بلستكة"
  }
];

export const blogPosts = [
  {
    id: 1,
    title: "كيف تبدأ رحلتك في تصميم المنتجات الرقمية من الصفر في 2026؟",
    category: "تصميم المنتجات",
    date: "25 مارس",
    readTime: "6 دقائق للقراءة",
    image: "https://images.unsplash.com/photo-1614850523296-d8c1af93d400?w=800&q=80"
  },
  {
    id: 2,
    title: "أهمية تجربة المستخدم (UX) في نجاح الشركات الناشئة ونموها",
    category: "تجربة المستخدم",
    date: "22 مارس",
    readTime: "8 دقائق للقراءة",
    image: "https://images.unsplash.com/photo-1614850523296-d8c1af93d400?w=800&q=80"
  },
  {
    id: 3,
    title: "دليلك الشامل لبناء نظام تصميم (Design System) مرن وقابل للتوسع",
    category: "أنظمة التصميم",
    date: "20 مارس",
    readTime: "10 دقائق للقراءة",
    image: "https://images.unsplash.com/photo-1614850523296-d8c1af93d400?w=800&q=80"
  },
  {
    id: 4,
    title: "كيف تختار الألوان والخطوط المناسبة لهوية علامتك التجارية؟",
    category: "الهوية البصرية",
    date: "18 مارس",
    readTime: "5 دقائق للقراءة",
    image: "https://images.unsplash.com/photo-1614850523296-d8c1af93d400?w=800&q=80"
  },
  {
    id: 5,
    title: "استراتيجيات تحويل الأفكار إلى منتجات رقمية ناجحة ومربحة",
    category: "استشارات الشركات",
    date: "15 مارس",
    readTime: "7 دقائق للقراءة",
    image: "https://images.unsplash.com/photo-1614850523296-d8c1af93d400?w=800&q=80"
  },
  {
    id: 6,
    title: "أدوات الذكاء الاصطناعي التي ستغير طريقة عمل المصممين في المستقبل",
    category: "تكنولوجيا",
    date: "12 مارس",
    readTime: "9 دقائق للقراءة",
    image: "https://images.unsplash.com/photo-1614850523296-d8c1af93d400?w=800&q=80"
  },
  {
    id: 7,
    title: "لماذا يعتبر البحث عن المستخدم (User Research) أهم خطوة في التصميم؟",
    category: "أبحاث المستخدم",
    date: "10 مارس",
    readTime: "6 دقائق للقراءة",
    image: "https://images.unsplash.com/photo-1614850523296-d8c1af93d400?w=800&q=80"
  },
  {
    id: 8,
    title: "كيفية تحسين معدل التحويل (CRO) من خلال تحسين واجهة المستخدم",
    category: "واجهة المستخدم",
    date: "8 مارس",
    readTime: "5 دقائق للقراءة",
    image: "https://images.unsplash.com/photo-1614850523296-d8c1af93d400?w=800&q=80"
  },
  {
    id: 9,
    title: "نصائح للمصممين المستقلين للتعامل مع العملاء وبناء بورتفوليو قوي",
    category: "العمل الحر",
    date: "5 مارس",
    readTime: "7 دقائق للقراءة",
    image: "https://images.unsplash.com/photo-1614850523296-d8c1af93d400?w=800&q=80"
  }
];

export const projects = [
  {
    name: "Ghadwa Platform & Ops",
    type: "Platform",
    country: "🇪🇬",
    roles: ["Product Lead", "Growth Ops"],
    tags: ["FoodTech", "Marketplace_Management", "Automation", "n8n"],
    desc: "منصة متكاملة لربط وإدارة الشيفات مع نظام أتمتة عالج +1,500 طلب وحقق 500 ألف ج.م إيرادات في 3 أشهر.",
    image: "https://i.ibb.co/C5RY3g3F/Gemini-Generated-Image-k1xgcbk1xgcbk1xg.png",
    link: "https://www.ghadwah.com/"
  },
  {
    name: "Rebekia: Waste-to-Value Platform",
    type: "Green-Tech Platform",
    country: "🇪🇬",
    roles: ["Founder & PM", "UI/UX Designer"],
    tags: ["Green_Tech", "Product_Design", "Fundraising", "Logistics"],
    desc: "منصة Green-Tech لرقمنة إدارة النفايات، حققت +3 مليون ج.م إيرادات وحصلت على منح بقيمة 2 مليون ج.م.",
    image: "https://i.ibb.co/b5cMK38H/480650020-560298037054530-297461002153503365-n.jpg",
    link: "https://www.facebook.com/search/top?q=rebekia%20-%20%D8%B1%D9%8A%D8%A8%D9%8A%D9%83%D9%8A%D8%A7%20"
  },
  {
    name: "Ibn Sina EdTech Scaling",
    type: "EdTech Platform",
    country: "🇪🇬",
    roles: ["Product Lead", "Growth Engineer"],
    tags: ["EdTech", "Product_Growth", "Automation", "User_Experience"],
    desc: "قيادة وتطوير منصة تعليمية تخدم +250 ألف متعلم، مع أتمتة الأنظمة.",
    image: "https://i.ibb.co/PLDVNVN/633063279-1305255694981643-264405767237916710-n.jpg",
    link: "https://ibnsinatrainingacademy.com/"
  },
  {
    name: "Kidihow: Skill-Building Platform",
    type: "EdTech Platform",
    country: "🇪🇬",
    roles: ["Product Manager", "UI/UX Lead"],
    tags: ["Child_Development", "Marketplace", "Product_Design", "Booking_Systems"],
    desc: "منصة متخصصة لربط أولياء الأمور بخبراء تنمية مهارات الأطفال، مع التركيز على تجربة مستخدم سلسة ونظام حجز مؤتمت.",
    image: "https://i.ibb.co/4gm3qDGZ/image.png"
  },
  {
    name: "Honna La Hona: Women Empowerment",
    type: "Platform",
    country: "🇪🇬",
    roles: ["Lead Product Designer", "UX Strategist"],
    tags: ["Social_Impact", "Product_Design", "Women_Empowerment", "UX_Research"],
    desc: "منصة متخصصة لدعم عمل المرأة من المنزل، تهدف لتمكين السيدات اقتصادياً من خلال توفير فرص عمل مرنة وواجهة مستخدم بسيطة وشاملة.",
    image: "https://i.ibb.co/ynnfCzJY/45.png"
  },
  {
    name: "Faseela: Eco-Friendly Decor",
    type: "E-commerce",
    country: "🇪🇬",
    roles: ["Lead Product Designer & manger", "E-commerce Strategist"],
    tags: ["E-commerce", "Sustainable_Design", "UI/UX", "Nature_Products"],
    desc: "متجر إلكتروني متخصص في الديكورات المستمدة من الطبيعة، يركز على تجربة تسوق بصرية مريحة تربط بين الحرف اليدوية وعشاق التصميم المستدام.",
    image: "https://i.ibb.co/pvnPDCG1/480932989-1459014528817597-3730280492153839194-n.jpg",
    link: "https://www.facebook.com/fasila.decorations"
  },
  {
    name: "Web Autopilot: AI Localization",
    type: "AI SaaS",
    country: "🇸🇦",
    roles: ["Product Designer (AI)", "Interaction Designer"],
    tags: ["AI_SaaS", "SEO_Optimized", "UX_Design", "Global_Scale"],
    desc: "أداة مدعومة بالذكاء الاصطناعي لترجمة المواقع وتدويلها (Globalization) مع الحفاظ التام على أداء الـ SEO وتجربة المستخدم الأصلية.",
    image: "https://i.ibb.co/JjMfwfF5/45.png",
    link: "https://webautopilot.ai/"
  },
  {
    name: "Scolifay: Gamified EdTech",
    type: "EdTech",
    country: "🇦🇪",
    roles: ["Interaction Designer", "Gamification Specialist"],
    tags: ["EdTech_Kids", "Gamification", "Interactive_Learning", "User-Centered_Design"],
    desc: "منصة تعليمية تفاعلية للأطفال تدمج بين الألعاب والأنشطة الأكاديمية (الرياضيات، العلوم، والقراءة) لتقديم تجربة تعلم ممتعة ومحفزة.",
    image: "https://i.ibb.co/JWzs5TZ2/5-5.png"
  },
  {
    name: "Basset: Sustainable Swapping",
    type: "Platform",
    country: "🇪🇬",
    roles: ["Product Designer", "User Experience (UX) Specialist"],
    tags: ["Circular_Economy", "P2P_Marketplace", "Sustainable_Living", "UI/UX"],
    desc: "منصة مصرية رائدة للمقايضة الذكية، تهدف لتقليل الهدر وتعزيز الاستهلاك المستدام من خلال ربط المستخدمين لتبادل السلع والمنتجات بسهولة وأمان.",
    image: "https://i.ibb.co/0yC3m8nq/5-5-5.png"
  }
];

export const experience = [
  {
    id: 1,
    role: "Chief Technology Officer",
    company: "Ghadwa app - غدوة",
    period: "أغسطس 2025 - الحالي",
    desc: "قيادة الجوانب التقنية لمتجر الأغذية الصحية وخدمة توصيل الطعام، مع التركيز على أتمتة العمليات وتحسين الكفاءة.",
    tags: ["Product Management", "Continuous Product Discovery"]
  },
  {
    id: 2,
    role: "Chief Executive Officer",
    company: "Rebekia - ريبيكيا",
    period: "مايو 2023 - الحالي",
    desc: "إدارة وتوجيه الرؤية الاستراتيجية لشركة ريبيكيا وقيادة الفريق نحو تحقيق أهداف النمو والابتكار.",
    tags: ["AI Product Management", "Leadership"]
  },
  {
    id: 3,
    role: "Product Delivery Manager",
    company: "Rebekia - ريبيكيا",
    period: "يونيو 2023 - الحالي",
    desc: "إدارة دورة حياة تسليم المنتجات وضمان الجودة والكفاءة في العمليات التقنية والتصميمية.",
    tags: ["Problem-Solving", "Time Management"]
  },
  {
    id: 4,
    role: "Product Designer",
    company: "Upwork",
    period: "فبراير 2024 - الحالي",
    desc: "تصميم منتجات رقمية متنوعة لعملاء عالميين، مع التركيز على تجربة المستخدم والبحث النوعي.",
    tags: ["UI/UX", "Interaction Design", "Wireframing"]
  },
  {
    id: 5,
    role: "Ai Product Designer",
    company: "WebAutoPilot",
    period: "فبراير 2025 - يونيو 2025",
    desc: "تصميم واجهات مستخدم ذكية تعتمد على تقنيات الذكاء الاصطناعي لتحسين تجربة القيادة والتحكم.",
    tags: ["AI Product Management", "Usability Testing"]
  },
  {
    id: 6,
    role: "Product Designer",
    company: "Operation Excellence (OPEX)",
    period: "يوليو 2024 - أغسطس 2024",
    desc: "تصميم حلول سحابية متقدمة وتطوير منصات إدارة البيانات للشركات الكبرى في السعودية.",
    tags: ["Cloud Solutions", "Product Design"]
  },
  {
    id: 7,
    role: "Product Designer",
    company: "Ibn Sina Training Academy",
    period: "أبريل 2024 - يوليو 2024",
    desc: "تصميم منصات تعليمية متخصصة في العلوم الحياتية والترجمة الطبية وتحسين رحلة المتعلم.",
    tags: ["UX Research", "UI Design"]
  },
  {
    id: 8,
    role: "Product Designer",
    company: "Camp Coding",
    period: "يوليو 2023 - سبتمبر 2023",
    desc: "المساهمة في مرحلة التفكير والبحث السوقي وتحويل احتياجات المستخدمين لواجهات برمجية بديهية.",
    tags: ["Design (UED)", "Agile Methodologies"]
  },
  {
    id: 9,
    role: "UI Designer",
    company: "Apps square",
    period: "يوليو 2022 - أغسطس 2022",
    desc: "تطبيق المعرفة النظرية في تصميم واجهات المستخدم والمشاركة في تحسين النماذج الأولية.",
    tags: ["UI Design", "Prototyping"]
  },
  {
    id: 10,
    role: "Product Designer",
    company: "kidihow",
    period: "أكتوبر 2023 - نوفمبر 2024",
    desc: "تصميم تجارب مستخدم مخصصة للأطفال وأولياء الأمور، مع التركيز على التفاعل والتعلم.",
    tags: ["Product Design", "UX Design"]
  },
  {
    id: 11,
    role: "Product Designer",
    company: "Honnalahona",
    period: "أبريل 2023 - مايو 2023",
    desc: "تصميم واجهات مستخدم لمنصة متخصصة في الخدمات اللوجستية وتوصيل الطلبات.",
    tags: ["UI/UX", "Logistics"]
  },
  {
    id: 12,
    role: "Product Designer",
    company: "Fasila",
    period: "مارس 2024 - نوفمبر 2024",
    desc: "تطوير الهوية البصرية وتصميم واجهات المستخدم لمنصة زراعية ذكية.",
    tags: ["Product Design", "AgriTech"]
  }
];
